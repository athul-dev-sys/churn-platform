...
export const RetentionActions: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    setPage(1);
    try {
      const res = await fetchCustomers(undefined, undefined, 1, 1000);
  ...
  const atRisk = useMemo(
    () => customers.filter((c) => c.scores?.[0]?.riskBand === 'High' || c.scores?.[0]?.riskBand === 'Medium'),
    [customers]
  );

  const totalPages = Math.ceil(atRisk.length / PAGE_SIZE) || 1;

  const currentAtRisk = useMemo(() => {
    const start = (page - 1) * PAGE_SIZE;
    return atRisk.slice(start, start + PAGE_SIZE);
  }, [atRisk, page]);

  const selectAllCurrent = (checked: boolean) => {
    const currentCustomerIds = currentAtRisk.map((c) => c.customerId);
    if (checked) {
      setSelectedIds((prev) => Array.from(new Set([...prev, ...currentCustomerIds])));
    } else {
      setSelectedIds((prev) => prev.filter((id) => !currentCustomerIds.includes(id)));
    }
  };
  ...
  const runBatch = async () => {
    ...
    try {
      ...
      setBatchError(err instanceof Error ? err.message : 'Batch prediction process failed.');
    } finally {
    ...
    <div className="space-y-6">
      <header className="border-b border-slate-200/60 pb-6">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <h1 className="page-title mt-2">Retention Action Center</h1>
            <p className="page-subtitle mt-2 max-w-2xl">
              Prioritize at-risk customers, review recommended campaigns and trigger fresh model scoring.
            </p>
          </div>
          <button
            onClick={() => void loadData()}
            disabled={loading}
            className="glass-button inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-xs font-bold"
          >
            <RefreshCw size={14} className={loading ? 'animate-spin' : ''} /> Reload system
          </button>
        </div>
      </header>
      ...
      {loading ? (
        <div className="glass-card rounded-3xl p-16 text-center">
          ...
          <p className="mt-4 text-sm font-semibold text-slate-600">Initializing action panel...</p>
        </div>
      ) : error ? (
        ...
        <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
          <section className="space-y-4">
            <div>
              <p className="eyebrow">Retention Playbook</p>
              <h2 className="mt-1 text-lg font-bold text-slate-900">Recommended Campaigns</h2>
            </div>
            <Campaign
              tone="amber"
          ...
          <section className="glass-card rounded-3xl p-5 sm:p-6 lg:col-span-2">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <h2 className="mt-1 text-lg font-bold text-slate-900">Scoring Command Console</h2>
                </div>
              </div>
              <span className="rounded-full bg-slate-100 px-3 py-1 text-[10px] font-bold text-slate-600">
                Total At risk: {atRisk.length.toLocaleString()}
              </span>
            </div>
            <p className="mt-3 text-xs leading-5 text-slate-500">
              Select High and Medium risk accounts and trigger the batch scoring endpoint.
            </p>
            ...
            {batchLoading && (
              <Alert
                ...
                title="Scoring pipeline active"
                text="Calling the backend batch scoring endpoint and refreshing customer scores."
              />
            )}
            ...
            {batchResult && (
              <Alert
                ...
                title="Scoring cycle complete"
                text={`Updated ${batchResult.processed} customer accounts.`}
              />
            )}
              ...
              <table className="w-full min-w-[720px] text-left text-xs">
                <thead className="bg-slate-50/80">
                  <tr>
                    ...
                    {['Customer', 'Risk', 'Churn Score', 'Monthly Bill', 'Profile'].map((h) => (
                      <th key={h} className="px-4 py-3 text-[9px] font-bold uppercase tracking-[.13em] text-slate-400">
                        {h}
                      </th>
                    ))}
                  </tr>
                  ...
                  {currentAtRisk.map((c) => {
                    ...
                    return (
                      <tr key={c.id} className="hover:bg-emerald-50/30">
                        ...
                        <td className="px-4 py-3 font-mono font-bold text-slate-700">{c.customerId}</td>
                        <td className="px-4 py-3">
                          <span
                            className={`rounded-full border px-2 py-1 text-[10px] font-bold ${
                              band === 'High' ? 'border-red-200 bg-red-50 text-red-700' : 'border-amber-200 bg-amber-50 text-amber-700'
                            }`}
                          >
                            {band}
                          </span>
                        </td>
                        <td className="px-4 py-3 font-bold text-slate-700">{score ? `${(score.score * 100).toFixed(0)}%` : '—'}</td>
                        <td className="px-4 py-3 text-slate-500">${c.monthlyCharges.toFixed(2)}</td>
                        <td className="px-4 py-3 text-right">
                          <Link to={`/customer/${c.customerId}`} className="font-bold text-emerald-600 hover:text-emerald-700">
                            Profile
                          </Link>
                        </td>
                      </tr>
                  ...
                  {currentAtRisk.length === 0 && (
                    <tr>
                      <td colSpan={6} className="px-4 py-10 text-center text-slate-400">
                        No High or Medium risk profiles are currently loaded.
                      </td>
                    </tr>
                  )}
                </tbody>
            ...
            {/* Pagination Controls */}
            {atRisk.length > 0 && (
              <div className="mt-4 flex flex-col gap-3 border-t border-[#eee8df] pt-4 sm:flex-row sm:items-center sm:justify-between">
                <p className="text-xs text-slate-500">
                  Showing <span className="font-bold text-slate-700">{startCount}</span>–
                  <span className="font-bold text-slate-700">{endCount}</span> of{' '}
                  <span className="font-bold text-slate-700">{atRisk.length.toLocaleString()}</span> at-risk accounts
                </p>
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => setPage((p) => Math.max(1, p - 1))}
                    disabled={page <= 1}
                    className="inline-flex items-center gap-1 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-bold text-slate-700 shadow-sm disabled:opacity-40"
                  >
...
function Alert({ tone, icon, title, text }: { tone: 'blue' | 'red' | 'green'; icon: React.ReactNode; title: string; text: string }) {
  ...
  return (
    <div className={`mt-4 flex items-start gap-3 rounded-2xl border p-4 ${c}`}>
      <span className="mt-0.5">{icon}</span>
      <div>
        <p className="text-xs font-bold">{title}</p>
        <p className="mt-1 text-[11px] opacity-80">{text}</p>
      </div>
    </div>
  );
}
...
function ErrorState({ message, onRetry }: { message: string; onRetry: () => void }) {
  return (
    <div className="glass-card rounded-3xl p-12 text-center">
      ...
      <h3 className="mt-4 text-sm font-bold text-slate-800">Connection Error</h3>
      <p className="mx-auto mt-2 max-w-md text-xs text-slate-500">{message}</p>
      <button onClick={onRetry} className="mt-4 rounded-xl bg-slate-900 px-4 py-2 text-xs font-bold text-white">
        Try again
      </button>
    </div>
  );
