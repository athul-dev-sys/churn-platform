...
def make_decision(
    ...
    """
    Convert churn probability and customer characteristics
...
def predict_churn(features: CustomerFeatures):
    ...
    try:
        # Convert the API request into the same raw feature format
        # that was used when training the preprocessing pipeline.
        ...
        # Convert Gender exactly as done during training
        customer_data["Gender"] = customer_data["Gender"].map({
            "Male": 1,
            "Female": 0
        })

        # Convert Yes/No fields exactly as done during training
        binary_columns = [
            "Under 30",
            "Senior Citizen",
            "Married",
        ...
        for column in binary_columns:
            customer_data[column] = customer_data[column].map({
                "Yes": 1,
                "No": 0
            })
